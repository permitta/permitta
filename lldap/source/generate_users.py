...
with open("permitta/mock_data/principals.json") as f:
...
with open("lldap/bootstrap/group-configs/groups.json", "w") as f:
    f.write(json.dumps({
        "name": "permitta_users_gl"
    }) + "\n")

    for group_prop in all_group_props:
        f.write(json.dumps({
            "name": group_prop.get("value")
        }) + "\n")

with open("lldap/bootstrap/user-configs/users.json", "w") as f:
    ...
    for principal in principals:
        ...
        ldap_user: dict = {
            ...
            "password": "changeme",
            "displayName": principal.get("first_name").title() + " " + principal.get("last_name").title(),
            "firstName": principal.get("first_name").title(),
            "lastName": principal.get("last_name").title(),
            "gravatar_avatar": "true",
            "groups": [
                group_prop["value"], "permitta_users_gl"
            ]
        }
