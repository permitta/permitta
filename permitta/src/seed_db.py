...
def get_principal_dbo(mock_data: dict) -> PrincipalDbo:
    ...
    principal_dbo.user_name = (
        mock_data.get("first_name") + mock_data.get("last_name")
    ).lower()
    principal_dbo.job_title = "Rubbish Collector"

    principal_dbo.principal_attributes = get_principal_attribute_dbos(
        mock_data=mock_data
    )
...
principals: list[PrincipalDbo] = [
    get_principal_dbo(mock_user) for mock_user in mock_users
]
