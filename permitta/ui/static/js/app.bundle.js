...
// EXTERNAL MODULE: ./static/js/sidebar.js
var sidebar = __webpack_require__(94);
// EXTERNAL MODULE: ./static/js/dark-mode.js
var dark_mode = __webpack_require__(650);
...
function nextClusterBreak(str, pos, includeExtending) {
    ...
    if (pos && surrogateLow(str.charCodeAt(pos)) && surrogateHigh(str.charCodeAt(pos - 1)))
        pos--;
    let prev = dist_codePointAt(str, pos);
    pos += codePointSize(prev);
    while (pos < str.length) {
        let next = dist_codePointAt(str, pos);
        if (prev == ZWJ || next == ZWJ || includeExtending && isExtendingChar(next)) {
            ...
            while (i >= 0 && isRegionalIndicator(dist_codePointAt(str, i))) {
                countBefore++;
                i -= 2;
            }
            if (countBefore % 2 == 0)
                ...
                if (oldAddr != null) {
                    ...
                    if (this.dependencies.every(dep => {
                        return dep instanceof Facet ? oldState.facet(dep) === state.facet(dep) :
                            dep instanceof dist_StateField ? oldState.field(dep, false) == state.field(dep, false) : true;
...
class dist_StateField {
    ...
    */
    static define(config) {
        let field = new dist_StateField(nextID++, config.create, config.update, config.compare || ((a, b) => a === b), config);
        if (config.provide)
...
class Configuration {
    ...
    static resolve(base, compartments, oldState) {
        ...
        for (let ext of flatten(base, compartments, newCompartments)) {
            if (ext instanceof dist_StateField)
                fields.push(ext);
            else
...
class dist_EditorState {
    ...
    */
    applyTransaction(tr) {
        ...
        if (!conf) {
            ...
            let intermediateState = new dist_EditorState(conf, this.doc, this.selection, conf.dynamicSlots.map(() => null), (state, slot) => slot.reconfigure(state, this), null);
        ...
        new dist_EditorState(conf, tr.newDoc, selection, startValues, (state, slot) => slot.update(state, tr), tr);
    }
    /**
    ...
    */
    toText(string) {
        return Text.of(string.split(this.facet(dist_EditorState.lineSeparator) || DefaultSplit));
    }
    /**
    ...
    */
    static create(config = {}) {
        ...
        let doc = config.doc instanceof Text ? config.doc
            : Text.of((config.doc || "").split(configuration.staticFacet(dist_EditorState.lineSeparator) || DefaultSplit));
        ...
        return new dist_EditorState(configuration, doc, selection, configuration.dynamicSlots.map(() => null), (state, slot) => slot.create(state), null);
    }
    /**
    The size (in columns) of a tab in the document, determined by
    the [`tabSize`](https://codemirror.net/6/docs/ref/#state.EditorState^tabSize) facet.
    */
    get tabSize() { return this.facet(dist_EditorState.tabSize); }
    /**
...
*/
dist_EditorState.tabSize = /*@__PURE__*/Facet.define({
...
its argument.
*/
dist_EditorState.phrases = /*@__PURE__*/Facet.define({
    compare(a, b) {
...
class MouseSelection {
    constructor(view, startEvent, style, mustSelect) {
        ...
        this.multiple = view.state.facet(dist_EditorState.allowMultipleSelections) && addsSelectionRange(view, startEvent);
...
class dist_EditorView {
    ...
    */
    ...
    showAnnouncements(trs) {
        ...
        for (let tr of trs)
            for (let effect of tr.effects)
                if (effect.is(dist_EditorView.announce)) {
                    if (first)
    ...
    mountStyles() {
        ...
        let nonce = this.state.facet(dist_EditorView.cspNonce);
...
*/
dist_EditorView.styleModule = styleModule;
/**
...
*/
dist_EditorView.inputHandler = inputHandler;
/**
...
*/
dist_EditorView.scrollHandler = scrollHandler;
/**
This facet can be used to provide functions that create effects
to be dispatched when the editor's focus state changes.
*/
dist_EditorView.focusChangeEffect = focusChangeEffect;
/**
...
line separately.
*/
dist_EditorView.perLineTextDirection = perLineTextDirection;
/**
...
*/
dist_EditorView.exceptionSink = exceptionSink;
/**
...
*/
dist_EditorView.mouseSelectionStyle = mouseSelectionStyle;
/**
...
*/
dist_EditorView.dragMovesSelection = dragMovesSelection$1;
/**
...
*/
dist_EditorView.clickAddsSelectionRange = clickAddsSelectionRange;
/**
...
*/
dist_EditorView.decorations = decorations;
/**
...
*/
dist_EditorView.outerDecorations = outerDecorations;
/**
...
regions.
*/
dist_EditorView.atomicRanges = atomicRanges;
/**
...
supported.)
*/
dist_EditorView.bidiIsolatedRanges = bidiIsolatedRanges;
/**
...
*/
dist_EditorView.scrollMargins = scrollMargins;
/**
...
true.
*/
dist_EditorView.darkTheme = darkTheme;
/**
Provides a Content Security Policy nonce to use when creating
the style sheets for the editor. Holds the empty string when no
nonce has been provided.
*/
dist_EditorView.cspNonce = /*@__PURE__*/Facet.define({ combine: values => values.length ? values[0] : "" });
/**
Facet that provides additional DOM attributes for the editor's
editable DOM element.
*/
dist_EditorView.contentAttributes = contentAttributes;
/**
Facet that provides DOM attributes for the editor's outer
element.
*/
dist_EditorView.editorAttributes = editorAttributes;
/**
An extension that enables line wrapping in the editor (by
setting CSS `white-space` to `pre-wrap` in the content).
*/
dist_EditorView.lineWrapping = /*@__PURE__*/dist_EditorView.contentAttributes.of({ "class": "cm-lineWrapping" });
/**
...
search match).
*/
dist_EditorView.announce = /*@__PURE__*/dist_StateEffect.define();
...
const handleKeyEvents = /*@__PURE__*/dist_Prec.default(/*@__PURE__*/dist_EditorView.domEventHandlers({
    keydown(event, view) {
...
function runHandlers(map, event, view, scope) {
    ...
    let charCode = dist_codePointAt(name, 0), isChar = codePointSize(charCode) == name.length && name != " ";
...
const hideNativeSelection = /*@__PURE__*/(/* unused pure expression or super */ null && (Prec.highest(/*@__PURE__*/dist_EditorView.theme(themeSpec))));

const setDropCursorPos = /*@__PURE__*/dist_StateEffect.define({
    map(pos, mapping) { return pos == null ? null : mapping.mapPos(pos); }
});
const dropCursorPos = /*@__PURE__*/dist_StateField.define({
    create() { return null; },
    update(pos, tr) {
        if (pos != null)
...
class MatchDecorator {
    ...
    */
    createDeco(view) {
        let build = new dist_RangeSetBuilder(), add = build.add.bind(build);
...
function rectangularSelection(options) {
    ...
    return dist_EditorView.mouseSelectionStyle.of((view, event) => filter(event) ? rectangleSelectionStyle(view, event) : null);
}
const keys = {
    Alt: [18, e => !!e.altKey],
...
function crosshairCursor(options = {}) {
    ...
    return [
        plugin,
        dist_EditorView.contentAttributes.of(view => { var _a; return ((_a = view.plugin(plugin)) === null || _a === void 0 ? void 0 : _a.isDown) ? showCrosshair : null; })
    ];
}
...
const baseTheme = /*@__PURE__*/dist_EditorView.baseTheme({
    ".cm-tooltip": {
        zIndex: 100,
        boxSizing: "border-box"
    },
...
const panelPlugin = /*@__PURE__*/dist_ViewPlugin.fromClass(class {
    ...
    provide: plugin => dist_EditorView.scrollMargins.of(view => {
    ...
    */
    name = "") {
        ...
        if (!dist_EditorState.prototype.hasOwnProperty("tree"))
            Object.defineProperty(dist_EditorState.prototype, "tree", { get() { return syntaxTree(this); } });
        this.parser = parser;
        this.extension = [
            language.of(this),
...
Language.state = /*@__PURE__*/dist_StateField.define({
...
const language = /*@__PURE__*/Facet.define({
    ...
    enables: language => [
        Language.state,
        parseWorker,
        dist_EditorView.contentAttributes.compute([language], state => {
            let lang = state.facet(language);
...
function getIndentation(context, pos) {
    if (context instanceof dist_EditorState)
...
state.
*/
const foldState = /*@__PURE__*/dist_StateField.define({
    create() {
    ...
    provide: f => dist_EditorView.decorations.from(f),
    toJSON(folded, state) {
        let ranges = [];
...
function announceFold(view, range, fold = true) {
    ...
    return dist_EditorView.announce.of(`${view.state.phrase(fold ? "Folded lines" : "Unfolded lines")} ${lineFrom} ${view.state.phrase("to")} ${lineTo}.`);
}
/**
...
const dist_baseTheme$1 = /*@__PURE__*/dist_EditorView.baseTheme({
    ".cm-foldPlaceholder": {
        backgroundColor: "#eee",
...
const treeHighlighter = /*@__PURE__*/dist_Prec.high(/*@__PURE__*/dist_ViewPlugin.fromClass(TreeHighlighter, {
    decorations: v => v.decorations
}));
/**
...
const dist_baseTheme = /*@__PURE__*/dist_EditorView.baseTheme({
...
const bracketMatchingState = /*@__PURE__*/dist_StateField.define({
...
const historyField_ = /*@__PURE__*/dist_StateField.define({
    create() {
...
function pageInfo(view) {
    ...
    if (selfScroll) {
        for (let source of view.state.facet(dist_EditorView.scrollMargins)) {
...
function cursorByPage(view, forward) {
    ...
    if (page.selfScroll) {
        ...
        if (startPos && startPos.top > scrollTop && startPos.bottom < scrollBottom)
            effect = dist_EditorView.scrollIntoView(selection.main.head, { y: "start", yMargin: startPos.top - scrollTop });
    }
...
function deleteBy(target, by) {
    ...
    target.dispatch(state.update(changes, {
        scrollIntoView: true,
        userEvent: event,
        effects: event == "delete.selection" ? dist_EditorView.announce.of(state.phrase("Selection deleted")) : undefined
    }));
    return true;
}
function skipAtomic(target, pos, forward) {
    if (target instanceof dist_EditorView)
        for (let ranges of target.state.facet(dist_EditorView.atomicRanges).map(f => f(target)))
...
const deleteByChar = (target, forward, byIndentUnit) => deleteBy(target, range => {
    ...
    if (byIndentUnit && !forward && pos > line.from && pos < line.from + 200 &&
        ...
        if (before[before.length - 1] == "\t")
            return pos - 1;
        let col = dist_countColumn(before, state.tabSize), drop = col % getIndentUnit(state) || getIndentUnit(state);
...
function newlineAndIndent(atEof) {
    return ({ state, dispatch }) => {
        ...
        let changes = state.changeByRange(range => {
            ...
            if (indent == null)
                indent = dist_countColumn(/^\s*/.exec(state.doc.lineAt(from).text)[0], state.tabSize);
...
const indentLess = ({ state, dispatch }) => {
    ...
    dispatch(state.update(changeBySelectedLine(state, (line, changes) => {
        ...
        if (!space)
            return;
        let col = dist_countColumn(space, state.tabSize), keep = 0;
...
let startState = dist_EditorState.create({
  doc: "Hello World",
  extensions: [keymap.of(defaultKeymap)]
})

let view = new dist_EditorView({
  state: startState,
